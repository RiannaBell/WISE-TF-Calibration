...
def bias_calc(FigNamestr, M_star, alpha, log_W, scatter_coefficients, Data, Mtot_W1, Mtot_err_W1, addition, LF_exclude, CF_exclude, slope, intercept, no_galaxies):
    ...
    bins_2 = np.histogram(np.array(log_W), 143)[1]
    ...
    with PdfPages(FigNamestr) as pdf:
        for i in index_to_plot:
            ...
            #
            # axs[0].bar(bin_centres[i], hist_raw[i], width=bin_width[i] - 0.01, color='green', alpha=0.6,
            #            yerr=hist_raw_errs[i],
            #            label='{}'.format(Id), ecolor='k', capsize=5, zorder=0)
            # axs[0].bar(bin_centres[i], hist_raw[i], width=bin_width[i] - 0.01, color='springgreen', alpha=0.2, zorder=1)
            ...
            if max_index > 2:
                ...
                chi_best1 = Chi2_LF(b[0:max_index - 1], h[0:max_index - 1], e[0:max_index - 1])[1]
            ...
            axs[0].set_xlabel(r'Absolute Magnitude (vega mags), $\rm M_{\rm Tot}$', fontfamily='serif', fontsize=20, labelpad=7)
            ...
            # axs[1].bar(bin_centres[i], hist_completeness[i], width=bin_width[i] - 0.01, color='palevioletred', alpha=0.8,
            #            yerr=hist_completeness_errs[i], ecolor='k', capsize=5, zorder=1)
            # # axs[1].bar(bin_centres[i], hist_completeness[i], width=bin_width[i] - 0.01, color='mediumpurple',
            # #            alpha=0.8, zorder=0)
            ...
            # order = [1, 0]
            # axs[1].legend([handles[idx] for idx in order], [labels[idx] for idx in order], loc="upper right", frameon=False,
            #            facecolor='white', framealpha=1, fontsize=15)
            ...
            axs[1].set_xlabel(r'Absolute Magnitude (vega mags), $\rm M_{\rm Tot}$', fontfamily='serif', fontsize=20, labelpad=7)

            # ==================================================================================================================
            g = Groups.get_group(Ids[i])
            Id = Ids[i]

            g1 = pd.DataFrame(g)

            g2 = g1.nsmallest(len(g1), 'log_W', keep='all')

            group = g2.reset_index().set_index(np.arange(min(g.index), max(g.index) + 1), drop=False)

            log_W = np.array(g["log_W"])
            log_W_err = np.array(g["log_W_err"])
            ...
            for n in range(50000):
                ...
                for b in range(len(bins_2) - 1):
                    ...
                    W = log_W[np.where(np.logical_and(log_W >= bins_2[b], log_W <= bins_2[b + 1]))[0]]
                    W_err = log_W_err[np.where(np.logical_and(log_W >= bins_2[b], log_W <= bins_2[b + 1]))[0]]
            ...
            # Now I want to turn this sample into a list of lists (each list will have a different length though so YEET

            # so make a loop to run over each COLUMN of the array and then turn each column into its own array
            bias = []
            ...
            M_err_group = np.reshape(np.array(Mtot_err_W1)[group['index'].values.tolist()], (len(group),))
            M_tot_group = np.reshape(np.array(Mtot_W1)[group['index'].values.tolist()], (len(group),))

            U_M_tot_corrected[group['index'].values.tolist()] = [sum(x) for x in zip(M_tot_group, list(-1 * bias))]
            U_M_err_corrected[group['index'].values.tolist()] = [np.sqrt(sum(x)) for x in
                                                                 zip(M_err_group ** 2, list((-0.02 * bias) ** 2))]

            bias_full_list[group['index'].values.tolist()] = [bias]
            ...
            axs[2].set_xlabel(r'$\log_{10}(W_{F50})$', fontfamily='serif', fontsize=21, labelpad=5)
            ...
            axs[3].plot([max(W_array) + 0.03, min(W_array) - 0.03],
                        ...
                        color='blue', ls='--',
                        label='M = {0:.2f} {1:.2f}'.format(FitObj.params[1], FitObj.params[0]) + r'$\log_{10}(W_{F50})$')
            ...
            axs[3].set_ylabel(r'Absolute Magnitude (vega mags), $\rm M_{\rm Tot}$', fontfamily='serif', fontsize=21, labelpad=2)
            axs[3].set_xlabel(r'$\log_{10}(W_{F50})$', fontfamily='serif', fontsize=21, labelpad=5)
